...
if (window.rbx.animCursorGetToggle) {
  window.rbx.animCursorGetToggle()
    .then((tg) => { animCursorEnabled = !!(tg && tg.enabled); refreshAnimToggleButton(); })
    .catch(() => {  });
}
...
async function renderAnimCursorPanel() {
  ...
  let activeInfo = null;
  try { activeInfo = await window.rbx.activeCursors(); } catch (_) {  }
