...
function registerIpcHandlers({ animCursor, getMainWindow, checkForUpdatesIfDue }) {
  _animCursor = animCursor;
  _getMainWindow = getMainWindow || (() => null);
  _checkForUpdatesIfDue = checkForUpdatesIfDue || (async () => null);
  ...
  // ---- güncelleme kontrolü ----
  // "Şimdi Kontrol Et" düğmesi force=true gönderir ve CHECK_INTERVAL_MS
  // sınırını yok sayar (bkz. main.js: checkForUpdatesIfDue).
  ipcMain.handle('app:check-for-updates', async (_e, force) => {
    try {
      return await _checkForUpdatesIfDue(!!force);
    } catch (err) {
      logError(err);
      return { available: false, error: err && err.message ? err.message : String(err) };
    }
  });
  ipcMain.handle('app:open-update-url', (_e, url) => {
    // Sadece GitHub'ın kendi domain'ine izin ver — renderer'dan keyfi adres açtırılmaz.
